<template >
  <el-container v-if="isshow" :style="constyle" class="list-container">
    ...
    <ZoneDictSearch ref="zoneDictSearch"></ZoneDictSearch>
  </el-container>
</template>

<script>
import ZoneDictEdit from './ZoneDictEdit.vue'
import ZoneDictSearch from './ZoneDictSearch.vue'
export default {
  name: 'ZoneDict',
  components: {
    ZoneDictEdit,
  ...
  methods: {
    ...
    // 操作方法
    dataRowEdit (row) {
      var comp = ZoneDictEdit
      this.openNewTabpage('', 'add', {
        component: comp,
        text: this.currentRow.zoneName,
        index: 'ZoneDictEdit_' + this.currentRow.zoneId.toString(),
        openParm: {
          record: row,
          winType: 'ZoneDictEdit',
          winIndex: 'ZoneDictEdit_' + this.currentRow.zoneId.toString()
        }
      })
    },
    dataRowAdd () {
    ...
    dataSearch () {
      let me = this
      this.$refs.zoneDictSearch.$emit('show', function (conditions) {
