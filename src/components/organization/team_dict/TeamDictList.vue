<template >
  <el-container v-if="isshow" :style="constyle" class="list-container">
    ...
    <TeamDictSearch ref="teamDictSearch"></TeamDictSearch>
  </el-container>
</template>

<script>
import TeamDictEdit from './TeamDictEdit.vue'
import TeamDictSearch from './TeamDictSearch.vue'
export default {
  name: 'TeamDict',
  components: {
    TeamDictEdit,
  ...
  methods: {
    ...
    // 操作方法
    dataRowEdit (row) {
      var comp = TeamDictEdit
      this.openNewTabpage('', 'add', {
        component: comp,
        text: this.currentRow.teamName,
        index: 'TeamDictEdit_' + this.currentRow.teamId.toString(),
        openParm: {
          record: row,
          winType: 'TeamDictEdit',
          winIndex: 'TeamDictEdit_' + this.currentRow.teamId.toString()
        }
      })
    },
    dataRowAdd () {
    ...
    dataSearch () {
      let me = this
      this.$refs.teamDictSearch.$emit('show', function (conditions) {
