export default {
  ...
  methods: {
    init (parm) {
      var me = this
      if (parm != null && parm.record != null && parm.record.teamId != null) {
    ...
    recordSubmit () {
      ...
      me.axiosPost(
        ...
        if (response.data.statusCode === 8200) {
          if (me.form.teamId == null || me.form.teamId === '') {
            me.form.teamId = response.data.data.text
          }
          me.formSaveCallback('TeamDict', me.form)
          me.fromDataLoading = false
          me.$message({
            message: '数据保存成功！！！',
            type: 'success'
