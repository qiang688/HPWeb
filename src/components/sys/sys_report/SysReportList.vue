<template >
  <el-container v-if="isshow" :style="constyle" class="list-container">
    ...
    <SysReportEdit ref="sysReportEdit"></SysReportEdit>
  </el-container>
</template>

<script>
import SysReportEdit from './SysReportEdit.vue'
...
export default {
  ...
  methods: {
    ...
    dataRowEdit (row) {
      ...
      this.$refs.sysReportEdit.$emit('open', parm)
    },
    dataRowAdd () {
      this.$refs.sysReportEdit.$emit('open')
    },
    dataDel (row, fun) {
      let me = this
      me.axiosPost(
    ...
    dataLoad () {
      ...
      me.dataloading = true
      me.axiosPost(
        '/healthsys/SysReport/getRecords',
        me.conditions
      ).then(function (response) {
